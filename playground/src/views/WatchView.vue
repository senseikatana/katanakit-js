...
function toggleName() {
	nested.value.user.name = nested.value.user.name === "KatanaKit" ? "Playground" : "KatanaKit";
}
...
<template>
	<div class="watch-view">
		<div class="section">
			...
			<p class="hint">The watcher fires every time you type (deep watch).</p>
		</div>

		<div class="section">
			<h3>Nested Object Watcher</h3>
			<div class="controls">
				<button @click="incrementCount">Count: {{ nested.user.count }}</button>
				<button @click="toggleName">Toggle Name</button>
			</div>
			<p class="hint">Deep watch detects changes in nested properties.</p>
		</div>
		...
		<div class="section">
			...
			<div class="log-panel">
				<div v-if="log.length === 0" class="log-empty">No changes yet. Modify the inputs above.</div>
