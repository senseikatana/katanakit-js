...
<template>
	<div class="blog-listing">
		...
		<button @click="blog.refetch()" :disabled="blog.isFetching">
			{{ blog.isFetching ? "Loading..." : "Refresh" }}
		</button>

		<div v-if="blog.isPending" class="loading">Loading posts...</div>

		<div v-else-if="blog.error" class="error">
			<p>Error: {{ blog.error.message }}</p>
			<button @click="blog.refetch()">Retry</button>
		</div>

		<ul v-else-if="blog.data" class="post-list">
			<li v-for="page in blog.data" :key="page.id" class="post-item">
