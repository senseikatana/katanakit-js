...
export async function generateStaticParams() {
	const posts = await useNotionListAllDatabasePages(process.env.NOTION_DATABASE_ID!);

	if (!posts.ok) return [];

	return posts.data.map((page) => ({
		slug: (page.properties.Slug as any)?.rich_text?.[0]?.plain_text ?? page.id,
	}));
}
	...
	return (
		<main>
			...
			<article>
				...
				{blocks.ok ? (
					blocks.data.map((block) => <NotionBlock key={block.id} block={block} />)
				) : (
