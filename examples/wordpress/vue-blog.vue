...
const categories = useQuery({
	...
	queryFn: useSafeQueryFn(() => useWpGetCategories({ per_page: 50 })),
...
<template>
	<div class="blog-listing">
		...
		<button @click="posts.refetch()" :disabled="posts.isFetching">
			{{ posts.isFetching ? "Loading..." : "Refresh" }}
		</button>

		<!-- Categories filter -->
		<nav v-if="categories.data?.length" class="categories">
			<strong>Categories:</strong>
			<a v-for="cat in categories.data" :key="cat.id" :href="`/blog/category/${cat.slug}`">
				{{ cat.name }}
			</a>
		</nav>

		<div v-if="posts.isPending" class="loading">Loading posts...</div>

		<div v-else-if="posts.error" class="error">
			<p>Error: {{ posts.error.message }}</p>
			<button @click="posts.refetch()">Retry</button>
		</div>

		<ul v-else-if="posts.data?.length" class="post-list">
			<li v-for="post in posts.data" :key="post.id" class="post-item">
				<img
