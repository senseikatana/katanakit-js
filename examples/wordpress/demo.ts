...
async function createPost(): Promise<void> {
	...
	const result = await useWpCreatePost({
		...
		content:
			"<p>This post was created using the WordPress adapter.</p><p>It supports <strong>HTML</strong> content.</p>",
		status: "draft",
...
async function listComments(postId: number): Promise<void> {
	...
	if (result.ok) {
		...
		for (const comment of result.data) {
			console.log(
				`  - [${comment.id}] ${comment.author_name}: ${comment.content.rendered.slice(0, 50)}...`,
			);
		}
	} else {
		console.error("  Error:", result.error.message);
