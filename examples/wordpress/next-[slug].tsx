...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
	...
	const featuredImage = (post.data as any)._embedded?.["wp:featuredmedia"]?.[0]?.source_url;
	const authorName = (post.data as any)._embedded?.author?.[0]?.name ?? "Unknown";
	...
	return (
		<main>
			...
			<article>
				...
				{featuredImage && <img src={featuredImage} alt={post.data.title.rendered} width={800} />}
