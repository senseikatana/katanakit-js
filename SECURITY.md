...
- **API registry (`INIT`)** is trusted configuration. Validate the origin of any
...
- **URL construction** (`FetchApiManager.BUILD_URL`) uses `encodeURIComponent` for
...
- **Worker cleanup** (`WorkerService.RUN`) terminates the worker and revokes its
...
### 1. Do not use `ENCRYPT` for credentials

`GeneratorService.ENCRYPT` is a one-way hash **demo** with a fixed default salt
...
Treat everything read from `GET_STORAGE` as untrusted. Use `SET_TEXT`/`textContent`
instead of `SET_HTML`/`innerHTML` when displaying stored data, otherwise a value
...
`ThemeService.INIT_THEME` reads the stored mode without validating it. Check that
