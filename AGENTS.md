...
- Docs: MkDocs Material in `content/` (`mkdocs serve`, `mkdocs build --strict` → `site/`). The API reference is TypeDoc-generated by `hooks/generate_api.py`; CI does NOT build docs (deploy workflow does).
...
- Docs: MkDocs Material (`mkdocs.yml`, `content/`). `content/docs/**` maps 1:1 to `/docs/**` URLs (landing at `content/index.md`); the API reference is TypeDoc-generated into `content/docs/api/` by `hooks/generate_api.py` (never hand-edit `content/docs/api/`). To add locales later, move each tree under `content/<lang>/` (`/[lang]/docs/*`).
...
- **Docs:** `katanakit-docs` project → `docs.senseikatana.com` (MkDocs build output `site/`; the old `guides/` Docusaurus workspace is gone)
- **Playground:** `katanakit-playground` project → `play.senseikatana.com`
- Deploy: `bun run playground:deploy` or `bun run cf:deploy` (runs `mkdocs build --strict` first). Wrangler is pinned in devDeps. The playground resolves `katanakit-js` from `dist/`, so deploy scripts build the package first.
...
- Never add non-Cloudflare hosting/deploy providers. Two documented exceptions: INSForge for the database fallback, and `mkdocs gh-deploy` (gh-pages branch) as a **temporary docs preview only** while MkDocs is evaluated — Cloudflare stays the production host.
